...
def test_block_assignment():
    ...
    assert not page_group.children[0].structure, (
        "figure's structure should begin with nothing in it"
    )
    assert not page_group.children[1].structure, (
        "text's structure should begin with nothing in it"
    )

    page_group.merge_blocks(provider_outputs, text_extraction_method="custom")

    assert len(page_group.children[0].structure) == 3, (
        "figure block should have just 3 words"
    )
    assert len(page_group.children[1].structure) == 12, (
        "text should have the remaining 12 words"
    )
