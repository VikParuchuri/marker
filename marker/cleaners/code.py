...
def identify_code_blocks(blocks: List[Page]):
    try:
        ...
        for page in blocks:
            ...
            for block in page.blocks:
                ...
                comment_lines = comment_count(
                    [line.prelim_text for line in block.lines]
                )
                is_code = [
                    len(block.lines) > 3,
        ...
        print(f"Error in identify_code_blocks: {str(e)}")
        raise


def indent_blocks(blocks: List[Page]):
    print("trying to ident code blocks")
    try:
        span_counter = 0
        for page in blocks:
            for block in page.blocks:
        ...
        print(f"Error in indent_blocks: {str(e)}")
        raise
