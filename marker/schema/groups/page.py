...
class PageGroup(Group):
    ...
    def merge_blocks(
        ...
        # Preferentially assign text to text blocks rather than figures
        figures_pass = [
            i
            for i, block in enumerate(valid_blocks)
            if block.block_type in [BlockTypes.Figure, BlockTypes.FigureGroup]
        ]
        main_pass = [i for i in range(len(valid_blocks)) if i not in figures_pass]

        # Try to assign lines by intersection
        # provider_line_idx -> (intersection_area: np.float64, block_id)
        total_max_intersections = {}
        ...
        for block_subset_indices in [main_pass, figures_pass]:
            ...
            max_intersections = self.compute_line_block_intersections(
                block_subset, provider_outputs
            )

            # remove already assigned in previous passes
            max_intersections = {
                k: v
                for k, v in max_intersections.items()
                if k not in total_max_intersections
            }
